...
# Khởi tạo các biến lưu trữ trong phiên làm việc
...
def generate_daily_words(api_key, target, level, topic):
    ...
    try:
        ...
        # Xử lý chuỗi JSON nếu AI trả về kèm thẻ markdown
        if "```json" in content:
        ...
        st.error(f"Lỗi khi gọi AI API hoặc parse JSON: {e}")
        return []
...
# --- XỬ LÝ API KEY THÔNG MINH BÊN SIDEBAR ---
with st.sidebar:
    st.header("⚙️ Cài đặt hệ thống")
    
    # Kiểm tra xem có API Key trong file secrets chưa
    if "MINIMAX_API_KEY" in st.secrets:
        api_key = st.secrets["MINIMAX_API_KEY"]
        st.success("✅ Đã kết nối API tự động (từ secrets)")
    else:
        api_key = st.text_input("🔑 Nhập OpenRouter API Key", type="password", help="Dùng để gọi Minimax AI")
        if not api_key:
            st.warning("⚠️ Chưa có API Key. Hãy nhập để tiếp tục.")
...
with tab2:
    ...
    if not api_key:
        st.error("Vui lòng cung cấp API Key ở menu bên trái để AI có thể tạo bài học.")
    else:
        if st.button("✨ Nhờ AI Tạo 5 Từ Mới (Dựa trên hồ sơ của bạn)", use_container_width=True):
            with st.spinner("🤖 AI đang phân tích và chọn 5 từ phù hợp nhất..."):
                ...
                if words and len(words) > 0:
                    ...
                    st.error("AI không trả về đúng định dạng JSON hoặc bị lỗi kết nối. Vui lòng thử lại.")
...
with tab3:
    ...
    if not st.session_state.daily_words:
        st.info("Hãy sang tab '5 Từ Hôm Nay' để tạo danh sách từ trước khi ôn tập.")
    else:
        ...
        if mode == "🗂️ Flashcard thông minh":
            ...
            # Khung Flashcard sử dụng Markdown an toàn (Không dùng component HTML phức tạp)
            st.markdown(f"""
            ...
            if st.session_state.show_answer:
                ...
                st.markdown("Đánh giá độ nhớ của bạn để AI lên lịch ôn tập (Spaced Repetition):")
                col1, col2, col3 = st.columns(3)
                
                if col1.button("🔴 Quên (Ôn lại ngay)"):
            ...
            if not api_key:
                st.warning("Cần API Key để sử dụng chế độ tạo câu hỏi bằng AI.")
            else:
                ...
                if "current_quiz" in st.session_state:
                    ...
                    # Tạo form cho quiz để tránh reset trang liên tục
...
with tab4:
    ...
    if not st.session_state.learning_history:
        st.info("Chưa có dữ liệu học tập. Hãy hoàn thành các bài ôn tập để biểu đồ xuất hiện nhé!")
    else:
        ...
        col2.metric("Tổng từ đang quản lý", str(len(st.session_state.learning_history)))
        col3.metric("Từ đã thành thạo (Retention > 50%)", str(words_mastered), f"+{words_mastered}")
        ...
        df['Điểm Retention (%)'] = df['Điểm Retention (%)'].clip(0, 100) # Chuẩn hóa điểm tối đa là 100%
        
        st.markdown("---")
        st.markdown("**📊 Biểu đồ tỷ lệ ghi nhớ của từng từ**")
